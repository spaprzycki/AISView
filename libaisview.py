...
def aisVesselInfoFetch(message, db_host, db_user, db_pass, db_name):
    ...
    # sql = "SELECT vessel.name AS name, vessel.callsign AS callsign, type.type_desc as type \
    #      FROM vessel JOIN type ON type.type_id = vessel.type WHERE vessel.mmsi = %s"
    try:
